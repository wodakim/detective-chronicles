...
export default function Game() {
  const { currentLocationId, locations, clues, discoverClue, setCurrentLocation, language } = useGameStore();
          ...
          <div className="relative rounded-xl overflow-hidden border border-[#333] shadow-2xl bg-[#1a1a1a]">
            ...
            <div className="absolute bottom-0 left-0 p-6 z-20">
              <div className="flex items-center gap-2 text-[#fbc02d] mb-2">
                ...
                <span className="text-sm font-bold tracking-widest uppercase">{t('game.current_location')}</span>
              </div>
              <h2 className="text-4xl font-bold font-mono text-white mb-2">{t(currentLocation.name)}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold text-[#f0f0f0] flex items-center gap-2">
                  ...
                  {t('game.observable_elements')}
                </h3>
                <Badge variant="outline" className="border-[#fbc02d] text-[#fbc02d]">
                  {locationClues.filter(c => c.isDiscovered).length} / {locationClues.length} {t('game.found')}
                </Badge>
              </div>
                  ...
                  >
                    <CardHeader className="pb-2">
                      <CardTitle className="text-lg text-[#f0f0f0] group-hover:text-[#fbc02d] transition-colors flex justify-between items-start">
                        {t(clue.title)}
                        {clue.isDiscovered && <Badge className="bg-[#fbc02d] text-black hover:bg-[#fbc02d]">{t('game.analyzed')}</Badge>}
                      </CardTitle>
                    ...
                    <CardContent>
                      <p className="text-sm text-[#ccc] line-clamp-2">
                        {clue.isDiscovered ? t(clue.description) : t('game.examine_clue')}
                      </p>
                    </CardContent>
                  </Card>
                ))}
            ...
            <div className="space-y-6">
              <Card className="bg-[#1f1f1f] border-[#333] h-full">
                <CardHeader>
                  <CardTitle className="text-[#f0f0f0]">{t('game.investigation_notes')}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <p className="text-sm text-[#888] italic">
                    {t('game.investigation_tip')}
                  </p>
                  <Separator className="bg-[#333]" />
                  <div className="space-y-2">
                    <h4 className="text-sm font-bold text-[#ccc]">{t('game.objectives')}</h4>
            ...
            <>
              <DialogHeader>
                <DialogTitle className="text-2xl font-mono text-[#fbc02d] flex items-center gap-3">
                  ...
                  {t(clues[selectedClueId].title)}
                </DialogTitle>
                <DialogDescription className="text-[#888] text-lg">
                  {t('game.evidence')} #{clues[selectedClueId].id.toUpperCase()}
                </DialogDescription>
              </DialogHeader>
